function [it, iterations, data, obj, convergence] = Interior_Points_LP_alt(iter, nlp,dim)
% Implementation of the Interior Point method for linear
% problems (LPs)
% Returns the final iterate it, the number of iterations executed
% iterations, the iteration log data, the final objective and the 
% final error term convergence
    ...
    while 1
        ...
        con = [data.(name).pri_fea, data.(name).dual_fea, data.(name).compl,it.mu];
        convergence = max(con);

        % convergence criterium
        if convergence < 1*10^(-4)
            data.success = 1;
            break
        end
        ...
        if it.mu > 1*10^50 || (alpha_pri < 1*10^(-25) && alpha_dual < 1*10^(-25)
